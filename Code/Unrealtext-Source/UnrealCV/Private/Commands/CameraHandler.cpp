...
void FCameraCommandHandler::RegisterCommands()
{
	...
	//Cmd = FDispatcherDelegate::CreateRaw(this, &FCameraCommandHandler::GetLitSequence);
	//CommandDispatcher->BindCommand("vget /camera/[uint]/litseq [str]", Cmd, "Get snapshots around camera.");
	...
	Cmd = FDispatcherDelegate::CreateRaw(this, &FCameraCommandHandler::GetPugAngleByCameraLocation);
	Help = "Get camera location [x, y, z]";
	CommandDispatcher->BindCommand("vget /camera/[uint]/pugAngleByLocation", Cmd, Help);
	...
	Cmd = FDispatcherDelegate::CreateRaw(this, &FCameraCommandHandler::GetCameraRotationWithDirection);
	Help = "Get camera rotation [pitch, yaw, roll]";
	CommandDispatcher->BindCommand("vget /camera/[uint]/rotation_withDirection", Cmd, Help);
  ...
  CommandDispatcher->BindCommand("vset /camera/[uint]/pose [float] [float] [float] [float] [float] [float]", Cmd, Help);
	...
	Help = "Add texts into the scene, and return raw binary image data, instead of the image filename";
	Cmd = FDispatcherDelegate::CreateLambda([this](const TArray<FString>& Args) { return AddTextAndGetPngBinary(Args, TEXT("lit")); });
	CommandDispatcher->BindCommand("vget /camera/[uint]/textlit png [str]", Cmd, Help);
...
FExecStatus FCameraCommandHandler::GetCameraRotationWithDirection(const TArray<FString>& Args)
{
	if (Args.Num() == 1)
	{
		bool bIsMatinee = false;
		...
		FVector CameraDirection = CameraRotation.RotateVector(FVector(1.0,0.0,0.0));

		FString Message = FString::Printf(TEXT("%.3f %.3f %.3f %.3f %.3f %.3f"), CameraRotation.Pitch, CameraRotation.Yaw, CameraRotation.Roll, CameraDirection.X, CameraDirection.Y, CameraDirection.Z);
...
FExecStatus FCameraCommandHandler::GetPugAngleByCameraLocation(const TArray<FString>& Args){
	if (Args.Num() == 1)
	{
		bool bIsMatinee = false;
...
/*FExecStatus FCameraCommandHandler::GetLitSequence(const TArray<FString>& Args)
{
	// parse texts
	FString textContents = Args[1];
	TArray<FString> textContentArray;
	textContents.ParseIntoArray(textContentArray,TEXT(","),true);
...
FExecStatus FCameraCommandHandler::AddTextAndGetPngBinary(const TArray<FString>& Args, const FString & ViewMode)
{
	//First, add text actors into the scene
	...
	// put all text actors into the scene
	//FVector textLocation;
	//float actorL = 15.0;
	for (int i = 0;i < textContentArray.Num();i++) {

		/*textLocation.X = CameraLocation.X;// +0.1*CameraRotation.Pitch;
		textLocation.Y = CameraLocation.Y;// +0.1*CameraRotation.Yaw;
		textLocation.Z = CameraLocation.Z;// +0.1*CameraRotation.Roll;

		textLocation.X += actorL * (cos(PAI * CameraRotation.Pitch / 180.0) * cos(PAI * CameraRotation.Yaw / 180.0));
		textLocation.Y += actorL * (cos(PAI * CameraRotation.Pitch / 180.0) * sin(PAI * CameraRotation.Yaw / 180.0));
		textLocation.Z += actorL * (sin(PAI * CameraRotation.Pitch / 180.0));*/
		//
		APugTextPawn *newTA = FUE4CVServer::Get().GetGameWorld()->SpawnActor<APugTextPawn>(APugTextPawn::StaticClass());
		//APugTextPawn *newTA = this->GetWorld()->SpawnActor<APugTextPawn>(APugTextPawn::StaticClass());
		//newTA->SetActorLocation(textLocation);
		//newTA->SetActorRotation(CameraRotation);
	}
	FString Message = FString::Printf(TEXT("hello, %d text"), textContentArray.Num());
	return FExecStatus::OK(Message);

	//Second, get png image.
	/*int32 CameraId = FCString::Atoi(*Args[0]);
