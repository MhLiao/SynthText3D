...
public:
	...
	FExecStatus GetPugAngleByCameraLocation(const TArray<FString>& Args);
	...
	FExecStatus GetCameraRotationWithDirection(const TArray<FString>& Args);
  ...
  FExecStatus	SetCameraPose(const TArray<FString>& Args);
	...
	/** Add text(s) to viewport and get raw binary image data*/
	//@author boyu song
	FExecStatus AddTextAndGetPngBinary(const TArray<FString>& Args, const FString& ViewMode);
