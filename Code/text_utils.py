...
def sample_weighted(p_dict):
    ps = list(p_dict.keys())
    #print([p_dict[p] for p in ps])
    return np.random.choice(ps,p=[p_dict[p] for p in ps])
...
def GetNumber():
    ...
    for i in range(random.randint(1, 8)):
        if random.random() < 0.85 or i <= 2:
            num_str += random.choice(num_set)
        else:
            num_str += random.choice(sym_set)
        if len(num_str) >=4 and random.random() < 0.4: break
    return num_str
...
class RenderFont(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        self.min_font_h = 8 #px : 0.6*12 ~ 7px <= actual minimum height
        self.max_font_h = 30 #px
        self.p_flat = 0.6
        # self.p_flat = 0.10
    ...
    def setStoredToPath(self,spath):
        self.StoredToPath = spath
        return #None

    def setCurCnt(self,cCnt):
        self.curCnt = cCnt
        return #None

    def setColor(self,cR,cG,cB):
        self.colorR=cR
        self.colorG=cG
        self.colorB=cB
        return
    ...
    def render_multiline(self,font,text):
        ...
        #print("fsize "+str(round(2.0*line_bounds.width))+" "+str(round(1.25*line_spacing*len(lines))))
            ...
            for ch in l: # render each character
                if ch.isspace(): # just shift
                    ...
                    try:
                        ...
                        print('error on font.render_to(surf, (x,y), ch)')
    ...
    #     line_bounds = font.get_rect(lines[np.argmax(lengths)])
    #     # fsize = (round(2.0*line_bounds.width), round(1.25*line_spacing*len(lines)))
    #     fsize = (round(1.25*line_spacing*len(lines)), round(2.0*line_bounds.width))
    #     #print("fsize "+str(round(2.0*line_bounds.width))+" "+str(round(1.25*line_spacing*len(lines))))
    #     surf = pygame.Surface(fsize, pygame.locals.SRCALPHA, 32)
    ...
    def get_nline_nchar(self,mask_size,font_height,font_width):
        ...
        """
        H,W = mask_size
        #print("font size"+str(font_height)+" "+str(font_width))
    ...
    def sample_font_height_px(self,h_min,h_max):
        if random.random() < self.p_flat:
            rnd = random.random()
        else:
            rnd = np.random.beta(2.0,2.0)

        h_range = h_max - h_min
        prob_small = random.random()
        if prob_small<0.2:
            h_range = h_range / 4.0
        elif prob_small<0.5:
            h_range = h_range / 2.0
    ...
    #@author:brawie song
    def getRandomTextImg(self,maskh,maskw):
        # np.random.seed(3)
        #print("Coming in GRTI")
        loop_count = 0
        while True:
            loop_count += 1
            if loop_count > 10:
            ...
            try:
                ...
                font = self.font_state.init_font(font)
                break
            except:
                print("font failed. retry...")
                continue
        ...
        #if max_font_h < self.min_font_h: # not possible to place any text here
            #return #None
        while max_font_h < self.min_font_h:
            H+=2
            W+=2
            max_font_h = min(0.9*H, (1/f_asp)*W/(self.min_nchar+1))
            max_font_h = min(max_font_h, self.max_font_h)
        ...
        while len(text)==0:
            ...
            if try_times>20:
                break
        # print("hahaha==="+str(maskh)+" "+str(maskw))
        # print len(text)
        # print(text)
        #print("=========")
        if(len(text)==0):
    ...
    def visualize_bb(self, text_arr, bbs):
        ...
        #print(ta.shape[1])
        #print(ta.shape[2])
        emptyImage=np.zeros((ta.shape[0],ta.shape[1],4), np.uint8)
        for i in range(ta.shape[0]):
            for j in range(ta.shape[1]):
                #print(ta[i][j])
                emptyImage[i][j][0]=int(self.colorB*(ta[i][j]/255.0)) #98.0;#115.0;#ta[i][j];
                emptyImage[i][j][1]=int(self.colorG*(ta[i][j]/255.0)) #108.8#135.0;#ta[i][j];
                emptyImage[i][j][2]=int(self.colorR*(ta[i][j]/255.0)) #100.6#139.0;#ta[i][j];
                if ta[i][j]==0:
                    emptyImage[i][j][3]=0
                else:
                    emptyImage[i][j][3]=255
        overall_trans = np.random.uniform(0.70, 0.90)
        pointwise_trans = np.random.uniform(0.95, 1.05, size=ta.shape) if random.random() < 0.3 else 1.0

        cv2.imwrite(self.StoredToPath+'fore_text'+str(self.curCnt)+'.png',emptyImage)
        cv2.imwrite(self.StoredToPath+'back_text'+str(self.curCnt)+'.png',(ta*np.clip(overall_trans*pointwise_trans, a_min=0.4, a_max=1.0)).astype(np.uint8))
...
class FontState(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        class fakefreq(object):
            ...
            def values(self):
                for c in string.printable:
                    yield 1. / len(string.printable)
        # get character-frequencies in the English language:
        with open(char_freq_path,'rb') as f:
            self.char_freq = fakefreq()#cp.load(f)

        # get the model to convert from pixel to font pt size:
        with open(font_model_path,'rb') as f:
            #self.font_model = cp.load(f)
            self.font_model = [10] * 10000

        # get the names of fonts to use:
        self.FONT_LIST = osp.join(data_dir, 'fonts/fontlist_sub.txt') # fontlist_sub has more ...
        self.fonts = [os.path.join(data_dir,'fonts',f.strip()) for f in open(self.FONT_LIST).readlines()]
        print(f"=== === === {len(self.fonts)} types of fonts found === === === ")
    ...
    def get_font_size(self, font, font_size_px):
        ...
        # print(font)
        # print(font.name)
        if font.name in self.font_model:
    ...
    def sample(self):
        ...
        """
        selected_id = int(np.random.randint(0, len(self.fonts)))
        selected_font = self.fonts[selected_id]
        # pdb.set_trace()
        print(selected_font.split('/')[-1])
        return {
            'font': selected_font,
...
class TextSource(object):
    ...
    def __init__(self, min_nchar, fn):
        ...
        with open(fn,'r') as f:
            ...
            self.vocab = list(set(' '.join(txt).split(' ')))
            self.vocab_short = list(filter(lambda x:len(x)<=5, self.vocab))
            self.txt = [ ]
            for _ in range(len(txt)):
                line = []
                for _ in range(random.randint(5, 10)):
                    line.append(random.choice(self.vocab))
                self.txt.append(' '.join(line))
            # self.txt = [l.strip() for l in f.readlines()]
            # self.txt = [l.split(' ') for l in self.txt]
            # for i in range(len(self.txt)):
            #     random.shuffle(self.txt[i])
            #     self.txt[i] = ' '.join(self.txt[i])
        ...
        # self.p_line_nword = [2,1.5,6]  # normal: (mu, std)
    ...
    def get_lines(self, nline, nword, nchar_max, f=0.35, niter=100):
        def h_lines(niter=100):
            ...
            while not np.all(self.is_good(lines,f)) and iter < niter:
                ...
                lines = [' '.join([random.choice([random.choice(self.vocab), random.choice(self.vocab_short)+' '+random.choice(self.vocab_short)]) if random.random() < 0.90 else GetNumber() for _ in range(random.randint(1, 5))]) for i in range(nline)]
            return lines
